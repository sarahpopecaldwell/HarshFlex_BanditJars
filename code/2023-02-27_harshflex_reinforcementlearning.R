...
with(dens, polygon(x=c(x[c(x1,x1:x2,x2)]), y= c(0, y[x1:x2], 0), col=alpha(col.pal[2],alpha = 0.6), border = NA))
legend("topright", c("Not Harsh", "Harsh"), col = c(alpha(col.pal[6],alpha = 0.6),alpha(col.pal[2],alpha = 0.6)), lwd = 6, bty="n", cex = 1.2)
...
par(new = TRUE)
plot(dens, xlim = c(0,1), ylim = c(0,10), type="n", ann = FALSE, bty = "n", yaxt = "n")
with(dens, polygon(x=c(x[c(x1,x1:x2,x2)]), y= c(0, y[x1:x2], 0), col=alpha(col.pal[6],alpha = 0.6), border = NA))
